...
maskDetect_path = os.path.join(base_dir + r'./model_data/mask2.pth')

#loading face detection model and mask detection model
print("loading models")
print(maskDetect_path)
maskDetectModel = models.resnet18(pretrained = False)
num_ftrs = maskDetectModel.fc.in_features
maskDetectModel.fc = nn.Linear(num_ftrs, 2)
maskDetectModel.load_state_dict(torch.load(maskDetect_path))
...
#function to detect mask with resnet
def detect_mask_with_resnet(faces):
    predictions = []
    image_transforms = data_transforms = transforms.Compose([
...
while(True):
    ...
    predictions=detect_mask_with_resnet(faces)
